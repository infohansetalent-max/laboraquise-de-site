...
Gebaut: `/wissen/` mit genau einem Artikel und `/wissen/warum-zahnaerzte-das-dentallabor-wechseln/` mit vollständigem Inhalt, Inhaltsnavigation, praktischen Darstellungen, acht ankreuzbaren Fragen und einem kopierbaren Gesprächseinstieg. Gestaltung nach Nutzerkorrektur direkt aus der bestehenden Startseite übernommen: schwebende Navigation, zweifarbige Wortmarke, Webflow-Schriftregeln, Breiten, Footer, Knöpfe und Originalfotografie. Die allgemeine base.css ist entfernt. Keine neue Abhängigkeit und kein CMS. Die Startseite bleibt das einzige kommerzielle Akquise-Ziel.
...
## Gestaltungskorrektur und Abnahmegrenze

Die erste Phase-3-Vorschau wurde vom Nutzer optisch abgelehnt. Die korrigierte Fassung orientiert sich jetzt an der vorhandenen Startseite; das bisherige eigenständige Layout ist gesichert und ersetzt. Die visuelle Übereinstimmung wurde anhand gerenderter Ansichten geprüft und durch identische gemeinsame Stilwerte gestützt. Funktionstests allein gelten nicht als Designbeleg. Vergleichsbilder und das aktualisierte lokale Paket liegen im Ausgabeverzeichnis dieser Aufgabe. Eine Zustimmung des Nutzers zur korrigierten Gestaltung steht damit noch nicht fest.
